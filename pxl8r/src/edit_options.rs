use crate::palettes::palettes::{map_to_palette1, map_to_palette2, Color, PALETTE};
use image::{DynamicImage, GenericImageView, ExtendedColorType};
use image::imageops::FilterType::Nearest;
use std::io::Write;
use anyhow::Context;
pub fn dither1(image: &image::DynamicImage) -> anyhow::Result<()> {
...
pub fn dither2(image: &image::DynamicImage) -> anyhow::Result<DynamicImage> {
    let image = image.to_rgb8();
    let (width, height) = image.dimensions();
    let mut buffer = image.into_raw();
